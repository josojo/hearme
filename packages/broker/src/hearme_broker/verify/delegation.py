...
Steps (ARCHITECTURE.md §5, in order):
...
This module handles 1 and 2 (and computes the canonical hash callers need
for the revocation lookup and the agent-signature step).
"""
...
def verify_delegation(
    ...
    *,
    ...
    caller can do the revocations DB lookup (step 3) and the agent-signature
    check (step 4) without recomputing.

    # STUB: v0 does NOT verify the embedded ``zkpassport_proof``. We trust
    # the phone to attest correctly. Real ZK verification lands in v0.2
    # (ARCHITECTURE.md §11).
    """
